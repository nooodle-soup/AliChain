...
int main() {
    ...
    if(!myfile.is_open()){
        cout << "Unable to open file on backend server C" << endl;
    }   
    string singleLog;    
    ...
    // create a server SOCKET for server C and server M
    int serverC_socket = createSocketUDP(SERVER_C);    
    int serverM_socket = createSocketUDP(SERVER_M);  

    // set server C and M addresses 
    struct sockaddr_in serverC_addr = setSockAddr(SERVER_C);
    struct sockaddr_in serverM_addr = setSockAddr(SERVER_M);

    bindSocket(serverC_socket, serverC_addr, SERVER_C);
    // bindSocket(serverM_socket, serverM_addr, "Server M");

    // bootup message for server C
    ...
    while(1)
    {
        ...
        int recv_status = recvfrom(serverC_socket, &command, sizeof(command), 0, (struct sockaddr *) &serverM_addr, &serverM_len);
        // cout << command << endl;
        if(recv_status == -1)
        {
        ...
        cout << "The serverC finished sending the response to the the Main Server." << endl;
