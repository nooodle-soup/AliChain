...
{	
	// Create socket descriptor for Client A
	int socket_descr = createSocketTCP(CLIENT_A);

	// Create socket variables for Client A
	struct sockaddr_in clientA_Addr = setSockAddr(CLIENT_A);
...
{	
    ...
    {
    	error("Error sending balance check request from Client A.");
    }
...
void txCoins(int socket_descr, char* user1, char* user2, char* amount)
{
    ...
    if (send(socket_descr, message, strlen(message), 0) == -1)
    {
    	error("Error sending transaction request from Client A.");
    }
...
{	
    ...
    if (send(socket_descr, message, strlen(message), 0) == -1)
    {
    	error("Error sending transaction list request from Client A.");
    }

    cout << "Client A sent a sorted list request to the main server." << endl;
...
{	
    ...
    if (send(socket_descr, message, strlen(message), 0) == -1)
    {
    	error("Error sending user stats request from Client A.");
    }
