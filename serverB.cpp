...
int main() {
    ...
    if(!myfile.is_open()){
        cout << "Unable to open file on backend server B" << endl;
    }   
    string singleLog;    
    ...
    // create a server SOCKET for server B and server M
    int serverB_socket = createSocketUDP(SERVER_B);    
    int serverM_socket = createSocketUDP(SERVER_M);  

    // set server B and M addresses
    struct sockaddr_in serverB_addr = setSockAddr(SERVER_B);
    struct sockaddr_in serverM_addr = setSockAddr(SERVER_M);

    bindSocket(serverB_socket, serverB_addr, SERVER_B);
    // bindSocket(serverM_socket, serverM_addr, "Server M");

    // bootup message for server B
    ...
    while(1)
    {
        ...
        int recv_status = recvfrom(serverB_socket, &command, sizeof(command), 0, (struct sockaddr *) &serverM_addr, &serverM_len);
        // cout << command << endl;
        if(recv_status == -1)
        {
        ...
        // // send query back to main server

        cout << "The serverB finished sending the response to the the Main Server." << endl;
